...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(vscode.commands.registerCommand('texpresso.showPreview', () => {
		if (true || activeEditor) {
			...
			<body>
				<script>
					...
					function sendSizeMessage() {
						// send resize message to texpresso
						const message = {
							type: 'resize',
					...
					window.addEventListener('resize', sendSizeMessage);
					sendSizeMessage();
			...
			panel.webview.onDidReceiveMessage(message => {
				if (texpresso && texpresso.stdin) {
					const texpressoMessage = [
						"move-window", 
						message.windowWidth - message.bodyWidth + 10, // space to see the resize handle
						message.windowTop,
						message.bodyWidth - 10,
						message.windowHeight - message.windowTop
					];
					texpresso.stdin.write(JSON.stringify(texpressoMessage) + '\n');
					// stay on top
					const alwaysOnTopMessage = [
						"stay-on-top",
						"t"
					];
					texpresso.stdin.write(JSON.stringify(alwaysOnTopMessage) + '\n');
				}
			});
		}
	}));
